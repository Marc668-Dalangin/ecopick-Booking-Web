...
$requests = $pickupController->listSellerRequests(Auth::userId(), $selectedStatus, $sortOrder);
$transactionSortOrder = strtoupper((string) ($_GET['sort_order'] ?? 'DESC')) === 'ASC' ? 'ASC' : 'DESC';
$transactions = $dashboardController->getSellerTransactionHistory(Auth::userId(), $transactionSortOrder);
...
<div class="card border-0 shadow-sm">
    <div class="card-body p-4 p-lg-5">
        ...
        <form method="GET" class="row g-3 align-items-end mb-4" aria-label="Filter current bookings">
            <div class="col-12 col-md-6 col-lg-5"><label class="form-label fw-semibold" for="booking-status">Status</label><select class="form-select" id="booking-status" name="status" onchange="this.form.submit()"><option value="">All statuses</option><?php foreach ($allowedStatuses as $status): ?><option value="<?php echo Validator::escape($status); ?>" <?php echo $selectedStatus === $status ? 'selected' : ''; ?>><?php echo Validator::escape($status); ?></option><?php endforeach; ?></select></div>
            <div class="col-12 col-md-6 col-lg-5"><label class="form-label fw-semibold" for="booking-sort">Sort by date</label><select class="form-select" id="booking-sort" name="sort" onchange="this.form.submit()"><option value="DESC" <?php echo $sortOrder === 'DESC' ? 'selected' : ''; ?>>Newest First</option><option value="ASC" <?php echo $sortOrder === 'ASC' ? 'selected' : ''; ?>>Oldest First</option></select></div>
            <?php if ($selectedStatus !== null): ?><div class="col-12 col-lg-2"><a class="btn btn-outline-secondary w-100" href="<?php echo APP_URL; ?>/user-junkshop/current-bookings.php">Clear filters</a></div><?php endif; ?>
        </form>
        ...
        <div id="current-bookings-list" aria-live="polite">
            ...
            <?php else: ?><?php foreach ($requests as $request): ?>
                <article class="booking-row border rounded-3 p-3 p-lg-4 mb-3" data-request-id="<?php echo (int) $request['id']; ?>">
                    <div class="d-flex justify-content-between align-items-start flex-wrap gap-3"><div><div class="small text-muted">Booking reference</div><h5 class="fw-bold mb-1"><a href="<?php echo APP_URL; ?>/user-junkshop/booking-details.php?id=<?php echo (int) $request['id']; ?>"><?php echo Validator::escape($request['booking_reference']); ?></a></h5><div class="small text-muted"><?php echo Validator::escape($request['materials_summary']); ?></div><div class="small mt-2"><span class="text-muted">Junkshop:</span> <strong><?php echo Validator::escape($request['junkshop_name'] ?? 'Junkshop'); ?></strong></div></div><span class="badge <?php echo $request['current_status'] === 'Declined' ? 'bg-danger' : ($request['current_status'] === 'Completed' ? 'bg-success' : (in_array($request['current_status'], ['Cancelled', 'Cancelled by Seller', 'Cancelled by Junkshop'], true) ? 'bg-secondary' : 'bg-warning text-dark')); ?>"><?php echo Validator::escape($request['current_status']); ?></span></div>
            ...
            <section class="mt-5" aria-labelledby="transaction-history-heading">
                ...
                <form method="GET" class="d-flex align-items-center gap-2 mb-3 flex-wrap">
                    ...
                    <select name="sort_order" id="sort_order" class="form-select form-select-sm auto-submit" onchange="this.form.submit()">
                        <option value="DESC" <?php echo $transactionSortOrder === 'DESC' ? 'selected' : ''; ?>>Newest First (Descending)</option>
                        <option value="ASC" <?php echo $transactionSortOrder === 'ASC' ? 'selected' : ''; ?>>Oldest First (Ascending)</option>
                    </select>
                </form>
