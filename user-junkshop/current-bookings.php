            ...
            <section class="mt-5" aria-labelledby="transaction-history-heading">
                ...
                <?php foreach ($transactions as $transaction): ?>
                    <article class="border rounded-3 p-3 mb-3">
                        ...
                        <div class="small text-muted mt-2">Junkshop: <strong><?php echo Validator::escape($transaction['junkshop_name'] ?? 'Junkshop'); ?></strong> · Final seller amount: <strong>₱<?php echo number_format((float)$transaction['final_seller_amount'], 2); ?></strong> · Method: <?php echo Validator::escape($transaction['payment_method'] ?? 'Pending'); ?></div>
...
window.addEventListener('DOMContentLoaded', function () {
    ...
    if (window.EcoPickLiveUpdates) window.EcoPickLiveUpdates.startPolling({ key: 'seller-current-bookings', url: apiUrl, interval: 5000, onSuccess: payload => { if (payload.success) { renderRequests(payload.data.requests); renderJunkshopNames(payload.data.requests); } } });
});
</script>
<?php
