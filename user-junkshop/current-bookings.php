...
<div class="card border-0 shadow-sm">
    <div class="card-body p-4 p-lg-5">
        <div class="d-flex justify-content-between align-items-start flex-wrap gap-3 mb-4">
            ...
            <a href="<?php echo APP_URL; ?>/user-junkshop/new-pickup-request.php" class="btn btn-primary"><i class="bi bi-plus-lg"></i> New pickup request</a>
        </div>
        ...
        <div id="current-bookings-list" aria-live="polite">
            <?php if (empty($requests)): ?><div class="empty-state"><div class="display-6 text-muted"><i class="bi bi-calendar2-x"></i></div><h5 class="mt-3 mb-2 fw-bold">No pickup requests yet</h5><p class="text-muted mb-3">Your submitted pickup requests will appear here.</p><a href="<?php echo APP_URL; ?>/user-junkshop/new-pickup-request.php" class="btn btn-outline-primary"><i class="bi bi-plus-lg"></i> Create your first request</a></div>
...
window.addEventListener('DOMContentLoaded', function () {
    ...
    function renderRequests(requests) {
        ...
        if (!requests.length) { list.innerHTML = '<div class="empty-state"><div class="display-6 text-muted"><i class="bi bi-calendar2-x"></i></div><h5 class="mt-3 mb-2 fw-bold">No pickup requests yet</h5><p class="text-muted mb-3">Your submitted pickup requests will appear here.</p><a href="<?php echo APP_URL; ?>/user-junkshop/new-pickup-request.php" class="btn btn-outline-primary"><i class="bi bi-plus-lg"></i> Create your first request</a></div>'; return; }
        list.innerHTML = requests.map(request => { const cancelled = request.current_status === 'Cancelled'; const cancellable = ['Pending Request', 'Matched', 'Accepted'].includes(request.current_status); return '<article class="booking-row border rounded-3 p-3 p-lg-4 mb-3" data-request-id="' + Number(request.id) + '"><div class="d-flex justify-content-between align-items-start flex-wrap gap-3"><div><div class="small text-muted">Booking reference</div><h5 class="fw-bold mb-1"><a href="<?php echo APP_URL; ?>/user-junkshop/booking-details.php?id=' + Number(request.id) + '">' + escapeHtml(request.booking_reference) + '</a></h5><div class="small text-muted">' + escapeHtml(request.materials_summary) + '</div></div><span class="status-badge ' + (cancelled ? 'rejected' : 'pending') + '">' + escapeHtml(request.current_status) + '</span></div><div class="row g-3 mt-1 small"><div class="col-md-3"><div class="text-muted">Estimated total</div><strong>' + Number(request.estimated_total_weight || 0).toFixed(2) + ' kg</strong></div><div class="col-md-3"><div class="text-muted">Preferred pickup</div><strong>' + escapeHtml(request.preferred_pickup_date) + '</strong><br>' + escapeHtml(request.preferred_pickup_time) + '</div><div class="col-md-4"><div class="text-muted">Location</div><strong>' + escapeHtml(request.barangay) + '</strong><br>' + escapeHtml(request.pickup_address) + '</div><div class="col-md-2"><div class="text-muted">Created</div><strong>' + escapeHtml(String(request.created_at || '').slice(0, 10)) + '</strong></div></div><div class="d-flex gap-2 justify-content-end mt-3"><a class="btn btn-sm btn-outline-primary" href="<?php echo APP_URL; ?>/user-junkshop/booking-details.php?id=' + Number(request.id) + '"><i class="bi bi-eye"></i> View details</a>' + (cancellable ? '<button type="button" class="btn btn-sm btn-outline-danger cancel-request" data-request-id="' + Number(request.id) + '" data-reference="' + escapeHtml(request.booking_reference) + '"><i class="bi bi-x-circle"></i> Cancel</button>' : '') + '</div></article>'; }).join('');
        bindCancelButtons();
    }
