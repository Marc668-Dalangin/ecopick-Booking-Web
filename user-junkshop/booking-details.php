...
<div class="row g-4">
    ...
    <div class="col-lg-8">
        <div class="card border-0 shadow-sm"><div class="card-body p-4 p-lg-5"><div class="d-flex justify-content-between align-items-start flex-wrap gap-3 mb-4"><div><p class="eyebrow mb-1">Booking tracking</p><h2 class="fw-bold mb-1"><?php echo Validator::escape($request['booking_reference']); ?></h2><p class="text-muted mb-0">Submitted <?php echo Validator::escape(date('M d, Y g:i A', strtotime($request['created_at']))); ?></p></div><span id="booking-current-status" class="status-badge <?php echo str_contains((string)$request['current_status'], 'Cancelled') ? 'rejected' : 'pending'; ?>"><?php echo Validator::escape($request['current_status']); ?></span></div>
            ...
            <hr class="my-4"><section aria-labelledby="estimated-transaction-heading"><h5 id="estimated-transaction-heading" class="fw-bold mb-3">Estimated transaction calculation</h5><div class="d-flex justify-content-between gap-3"><span>Estimated recyclable value</span><strong>₱<?php echo number_format((float) $request['estimated_recyclable_value'], 2); ?></strong></div><div class="d-flex justify-content-between gap-3"><span>Pickup / Collection fee</span><strong class="text-danger">- ₱<?php echo number_format((float) $request['pickup_fee'], 2); ?></strong></div><div class="d-flex justify-content-between gap-3"><span>Ecopick service fee</span><strong class="text-danger">- ₱<?php echo number_format((float) $request['ecopick_service_fee'], 2); ?></strong></div><hr class="my-2"><div class="d-flex justify-content-between gap-3"><strong>Estimated net amount to receive</strong><strong>₱<?php echo number_format((float) $request['estimated_net_amount'], 2); ?></strong></div></section>
        </div></div>
    </div>
