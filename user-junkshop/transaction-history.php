...
$transactions = $controller->getSellerCompletedTransactions(Auth::userId());
        ...
        <?php else: ?>
            <?php foreach ($transactions as $transaction): ?>
                <article class="border rounded-3 p-3 p-lg-4 mb-3">
                    ...
                    <div class="row g-3 mt-2 small"><div class="col-md-3"><div class="text-muted">Completed</div><strong><?php echo Validator::escape(date('M d, Y H:i', strtotime($transaction['completed_at']))); ?></strong></div><div class="col-md-3"><div class="text-muted">Actual weight</div><strong><?php echo number_format((float)$transaction['actual_weight_kg'], 2); ?> kg</strong></div><div class="col-md-3"><div class="text-muted">Final recyclable value</div><strong>₱<?php echo number_format((float)$transaction['final_recyclable_value'], 2); ?></strong></div><div class="col-md-3"><div class="text-muted">Final payout</div><strong class="text-success">₱<?php echo number_format((float)$transaction['final_seller_amount'], 2); ?></strong></div></div>
                    ...
                    <?php if (($transaction['payment_method'] ?? '') === 'GCash' && ($transaction['payment_status'] ?? '') === 'Unpaid'): ?><a class="btn btn-sm btn-outline-primary mt-3" href="<?php echo APP_URL; ?>/user-junkshop/current-bookings.php">Upload GCash proof</a><?php elseif (!empty($transaction['payment_proof_id'])): ?><a class="btn btn-sm btn-outline-secondary mt-3" target="_blank" rel="noopener" href="<?php echo APP_URL; ?>/user-junkshop/api/payment.php?action=view-proof&amp;proof_id=<?php echo (int)$transaction['payment_proof_id']; ?>">View payment proof</a><?php endif; ?>
                </article>
            <?php endforeach; ?>
        <?php endif; ?>
    </div>
