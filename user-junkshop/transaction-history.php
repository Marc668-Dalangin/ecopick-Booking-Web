        ...
        <?php else: ?>
            <?php foreach ($transactions as $transaction): ?>
                <article class="border rounded-3 p-3 p-lg-4 mb-3">
                    ...
                    <div class="d-flex justify-content-between flex-wrap gap-2"><div><div class="small text-muted">Booking reference</div><h5 class="fw-bold mb-1"><?php echo Validator::escape($transaction['booking_reference']); ?></h5><div class="small text-muted"><?php echo Validator::escape($transaction['materials_summary'] ?? ''); ?></div><div class="small mt-2"><span class="text-muted">Junkshop:</span> <strong><?php echo Validator::escape($transaction['junkshop_name'] ?? 'Junkshop'); ?></strong></div></div><span class="badge text-bg-<?php echo ($transaction['payment_status'] ?? '') === 'Paid' ? 'success' : 'warning'; ?>"><?php echo Validator::escape($transaction['payment_status'] ?? 'Unpaid'); ?></span></div>
