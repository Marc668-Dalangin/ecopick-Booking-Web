    ...
    public function getPendingJunkshopRequests(int $junkshopId, ?string $selectedStatus = null, string $sortOrder = 'DESC'): array
    {
        ...
        return $this->db->query(
            'SELECT
                ...
                GROUP_CONCAT(CONCAT(COALESCE(NULLIF(rm.name, \'\'), rm.material_name), " (", FORMAT(COALESCE(pri.estimated_weight_kg, pri.estimated_weight), 2), " kg)") ORDER BY COALESCE(NULLIF(rm.name, \'\'), rm.material_name) SEPARATOR ", ") AS materials_summary,
                GROUP_CONCAT(CONCAT(pri.id, ":", COALESCE(NULLIF(rm.name, \'\'), rm.material_name), ":", FORMAT(COALESCE(pri.estimated_weight_kg, pri.estimated_weight), 2), ":", FORMAT(COALESCE(jmp.buying_price, 0), 2), ":", IF(jmp.material_id IS NULL, 0, 1)) ORDER BY COALESCE(NULLIF(rm.name, \'\'), rm.material_name) SEPARATOR "|") AS settlement_items,
