...
$startDate = (string) ($_GET['start_date'] ?? date('Y-m-01')); $endDate = (string) ($_GET['end_date'] ?? date('Y-m-d')); $sortOrder = (isset($_GET['sort_order']) && strtoupper((string) $_GET['sort_order']) === 'ASC') ? 'ASC' : 'DESC'; $report = (new AdminFeatureController())->getReport($startDate, $endDate, $sortOrder); $pageTitle = 'Reports'; $activePage = 'reports'; ob_start();
?>
