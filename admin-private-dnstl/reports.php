...
<div class="modal fade" id="globalReceiptModal" tabindex="-1" aria-labelledby="globalReceiptModalLabel" aria-hidden="true"><div class="modal-dialog modal-lg modal-dialog-centered"><div class="modal-content"><div class="modal-header"><h5 class="modal-title" id="globalReceiptModalLabel">GCash Receipt</h5><button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button></div><div class="modal-body text-center"><img id="globalReceiptModalImage" class="img-fluid" alt="GCash receipt"></div></div></div></div>
<script>
	const globalReceiptModal = document.getElementById('globalReceiptModal');
	const globalReceiptModalImage = document.getElementById('globalReceiptModalImage');
	document.addEventListener('click', function (event) {
		const receiptButton = event.target.closest('[data-receipt-url]');
		if (receiptButton) {
			globalReceiptModalImage.src = receiptButton.getAttribute('data-receipt-url');
		}
	});
	globalReceiptModal.addEventListener('hidden.bs.modal', function () {
		globalReceiptModalImage.removeAttribute('src');
	});
</script>
