...
$approvedJunkshops = $controller->listApprovedJunkshops();
$pageTitle = 'Approved Junkshops';
$activePage = 'approved-junkshops';
ob_start();
?>
<div class="card border-0 shadow-sm">
    ...
    <?php if ($feedback): ?><div class="alert alert-<?php echo $feedback['success'] ? 'success' : 'danger'; ?>" role="alert"><?php echo Validator::escape($feedback['message']); ?></div><?php endif; ?>
        <div class="d-flex justify-content-between align-items-center mb-4 flex-wrap gap-3">
            <div>
                <h4 class="mb-1 fw-bold"><i class="bi bi-shop-window"></i> Approved Junkshops</h4>
                <p class="text-muted mb-0">Verified partner junkshops currently active on the platform.</p>
            </div>
            <span class="badge bg-success-subtle text-success"><?php echo count($approvedJunkshops); ?> approved</span>
        </div>
        <?php echo CSRF::field(); ?>
        ...
        <?php if (empty($approvedJunkshops)): ?>
            <div class="empty-state">
                ...
                <h5 class="mt-3 mb-2 fw-bold">No approved junkshops</h5>
                <p class="text-muted mb-0">There are no approved junkshops to display yet.</p>
            </div>
        <?php else: ?>
                ...
                <table class="table table-hover align-middle">
                    ...
                    <tbody>
                        <?php foreach ($approvedJunkshops as $junkshop): ?>
                            <tr data-account-row data-account-id="<?php echo (int) $junkshop['account_id']; ?>">
                                ...
                                <td><span class="status-badge approved"><?php echo Validator::escape(strtoupper($junkshop['status'] ?? 'approved')); ?></span></td>
                                <td>
                                    <?php $accountStatus = strtolower((string) ($junkshop['account_status'] ?? 'active')); ?>
                                    <span class="status-badge <?php echo $accountStatus === 'active' ? 'approved' : 'pending'; ?> d-inline-block mb-2" data-account-status><?php echo Validator::escape($accountStatus); ?></span><br>
...
<div class="modal fade" id="confirmJunkshopAccountDeleteModal" tabindex="-1" aria-labelledby="confirmJunkshopAccountDeleteModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="confirmJunkshopAccountDeleteModalLabel">Delete account</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
...
window.addEventListener('DOMContentLoaded', function () {
    ...
    const confirmation = window.ecopick.setupActionConfirmation({ modalId: 'confirmJunkshopAccountDeleteModal' });
